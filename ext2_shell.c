...
int	fs_df( DISK_OPERATIONS* disk, SHELL_FILESYSTEM* fs)
{
	...
	used_per = (float)used/(float)blocks * 100;
	free_per = (float)free/(float)blocks * 100;

	printf("\nFILE SYSTEM      BLOCKS      USED      USED%%      FREE      FREE%% \n");
	printf("%11s      %6u      %4u      %5.2f%%      %4u      %5.2f%%\n", fs->name, blocks, used, used_per, free, free_per);
	printf("\nblock size	%u (byte)\n", (0x01 << sb.log_block_size)*1024);
